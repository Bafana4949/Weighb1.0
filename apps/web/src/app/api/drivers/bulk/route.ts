...
export const POST = withScopeErrors(async function POST(request: Request) {
  ...
  for (let i = 0; i < parsed.data.rows.length; i++) {
    ...
    const row = driverSchema.safeParse({ ...raw, consent: true });
    if (!row.success) {
    ...
    const { idNumber, consent, ...data } = row.data;
    try {
      ...
      errors.push({ row: rowNumber, message: "Driver ID, RFID tag, or licence number is already registered" });
    }
  }
