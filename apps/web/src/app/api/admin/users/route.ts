...
export async function POST(request:Request){
  ...
  try {
    const user=await prisma.user.create({data:{...profile,email,organisationId:finalOrgId,passwordHash:await bcrypt.hash(password,12),roleAssignments:roleIds&&roleIds.length>0?{create:roleIds.map(roleId=>({roleId}))}:undefined}});
    await audit({userId:access.session!.user.id,action:"USER_CREATED",entityType:"user",entityId:user.id,afterData:{email:user.email,role:user.role,organisationId:user.organisationId,roleIds}});
