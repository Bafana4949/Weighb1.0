...
export default async function Sites({ searchParams }: { searchParams: Promise<{ page?: string; q?: string }> }) {
  ...
  const [sites, total, organisations] = await Promise.all([
    prisma.site.findMany({ where, include: { organisation: true, config: true, lanes: { orderBy: { laneNumber: "asc" } }, hardwareDevices: { include: { calibrationCertificates: { orderBy: { expiresAt: "desc" }, take: 1 } } } }, orderBy: { code: "asc" }, skip: (page - 1) * limit, take: limit }),
